...
- **Per-widget toggles belong in the `WidgetConfigurationIntent`, not app `AppSettings`**: A setting that controls one widget's display (e.g. "show birthdays in empty space") should be an `@Parameter(default:)` on the widget's `AppIntentConfiguration` intent — editable via long-press → Edit Widget — so each placed instance decides. Thread it through the TimelineProvider → loadSnapshot → Snapshot. Don't put it in app-wide `AppSettings` (wrong place, and creates a duplicate control). App-wide behaviors still belong in AppSettings; per-instance widget behaviors don't. From #329 QA
...
- **`.bottomBar` toolbar items are obscured by the iOS 26 floating tab bar on pushed views inside a TabView**: A drill-in screen (e.g. a Settings sub-screen pushed in a NavigationStack) that puts bulk actions in `ToolbarItem(placement: .bottomBar)` has them overlapped by the floating tab bar. Fix: add `.toolbar(.hidden, for: .tabBar)` to the pushed view so the bottom bar renders unobstructed. App-wide bulk actions that must coexist with the tab bar use `SelectionActionBar` via `.safeAreaInset(edge: .bottom)` at the TabView level instead. From #334 QA
...
- **App Group files read by the widget on the lock screen need `.completeFileProtectionUntilFirstUserAuthentication`, NOT `.complete`**: Widgets render on the Lock Screen while the device is locked. A PII file written with `.complete` is unreadable until unlock, breaking lock-screen widget reads. Pin the same class the Core Data store uses (`completeUntilFirstUserAuthentication`) so data stays encrypted at rest yet readable after first unlock. Don't rely on the OS default — pin it explicitly to match the app's posture
- **Widget date-relative copy needs a midnight-boundary timeline entry**: "tomorrow"→"today" / daysOverdue increments only roll at local midnight if the timeline emits an entry at `WidgetDataProvider.nextLocalMidnight(after:)` with `policy: .after(midnight)` and a snapshot computed *as-of* that midnight. A periodic `.after(now+30min)` masks the lag but isn't aligned. Repository saves already fire `WidgetRefresher` for data changes, so the periodic heartbeat isn't needed for those
- **Xcode rewrites the synchronized-group pbxproj on build**: Moving a file into `Shared/` (or any synced group) and then building causes Xcode to reorganize `project.pbxproj` (renames the group, adds a "Recovered References" group). Benign — `fileSystemSynchronizedGroups` still maps the path to both targets and build/tests pass. Commit the churn; don't fight it
...
- **Releases need BOTH build settings bumped in `project.pbxproj`, not the git tag**: The archive's `CFBundleShortVersionString` (what Xcode Organizer + App Store Connect show) comes from `MARKETING_VERSION`; `CFBundleVersion` comes from `CURRENT_PROJECT_VERSION`. The git tag only feeds the in-app `GeneratedVersion.swift` About-screen label via the build phase. Cutting a release = global-replace both settings (each appears 8x in `project.pbxproj`: Debug/Release x App/Widget/Tests/UITests), THEN tag. Bumping only the tag leaves archives reporting the old version
